<%-- 
    ...
    Created on : 26-Jan-2016, 19:43:54
    Author     : Ben
--%>
...
<body id="profile">
    ...
    <!-- Page Content -->
    <div class="container">
        <% Band band = (Band)session.getAttribute("SKVIEWUSER"); %>
        <h1>${SKVIEWUSER.getUsername()} Profile</h1>
        
        <div class="sideContent">
            <img src="${SKVIEWUSER.getPicDir()}" alt="profile picture">
            <h3>${SKVIEWUSER.getBandName()}</h3>
            <p>${SKVIEWUSER.getBio()}</p>
        </div>
        <div class="content">
            <div class = "sort">
                
                <p>Showing gigs created by ${SKVIEWUSER.getUsername()}</p>
            </div>
            <% Vector<Gig> getAllGigs = (Vector<Gig>) session.getAttribute("GIGSUSER");%>
            
            <c:forEach items="${GIGSUSER}" var="tempuser">
