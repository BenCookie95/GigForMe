...
public class GigsServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (null == session.getAttribute("USER")) {
            ...
            Vector<Gig> allGigsVect = gDAO.getAllTheGigs(0);
            request.getSession(true).setAttribute(Constants.SESSION_KEY_ALLGIGS, allGigsVect);
        }else{
            User getUser = (User) session.getAttribute("USER");
            Vector<Gig> allGigsVect = gDAO.getAllTheGigs(getUser.getUserID());
            request.getSession(true).setAttribute(Constants.SESSION_KEY_ALLGIGS, allGigsVect);
        }
        ...
        RequestDispatcher rd = request.getRequestDispatcher("gigs.jsp");
